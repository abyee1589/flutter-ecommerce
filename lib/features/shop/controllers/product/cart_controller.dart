...
class CartController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    loadCartItems(); // ✅ run this after controller is initialized
  }

  void addToCart(ProductModel product) {
    if (productQuantityInCart.value < 1) {
      AbLoaders.customToast(message: 'Select Quantity!');
      return;
    }

    if (product.productType == ProductType.variable.toString() &&
        variationController.selectedVariation.value.id.isEmpty) {
      AbLoaders.customToast(message: 'Select Variation!');
      return;
    }
    ...
    int index = cartItems.indexWhere(
      (cartItem) =>
          ...
          mapEquals(cartItem.selectedVariation ?? {}, selectedCartItem.selectedVariation ?? {}),
    );
    if (index >= 0) {
    ...
    updateCart();
    AbLoaders.customToast(message: 'Product added to cart successfully!');
  }
  ...
  void removeFromCartDialog(int index) {
    Get.defaultDialog(
      ...
      onConfirm: () {
        cartItems.removeAt(index);
        updateCart();
        AbLoaders.customToast(message: 'Product removed from cart!');
        Get.back();
      },
      onCancel: () =>
          () => Get.back(),
  ...
  void saveCartItems() {
    ...
    AbLocalStorage.instance().writeData('cartItems', cartItemStrings);
  }

  void loadCartItems() {
    final cartItemStrings = AbLocalStorage.instance().readData<List<dynamic>>(
      'cartItems',
    );
